...
function InvoiceCardComponent({
  ...
  return (
    <Animated.View
      entering={FadeInDown.delay(delay).duration(animation.duration.normal)}
    >
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={`Invoice ${invoice.id}`}
        onPress={onPress}
      ...
      >
        <Surface style={styles.card}>
          ...
          <View style={styles.copy}>
            <AppText style={styles.id}>{invoice.id}</AppText>
...
const styles = StyleSheet.create({
  ...
  copy: { flex: 1 },
  id: { ...typography.label, color: colors.text },
