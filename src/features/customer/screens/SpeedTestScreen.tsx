...
export default function SpeedTestScreen() {
  ...
  if (!environment.useMockApi) {
    return (
      <AppScreen contentContainerStyle={styles.content}>
        <AppHeader title="Speed test" showBack />
        <Surface style={styles.hero}>
          ...
          <AppText style={styles.hint}>
            Live speed testing is unavailable until a secure backend monitoring
            contract exists.
          </AppText>
        </Surface>
      </AppScreen>
  ...
  return (
    <AppScreen contentContainerStyle={styles.content}>
      <AppHeader
        title="Speed test"
        subtitle="Measure your connection performance"
        showBack
      />
      <Surface style={styles.hero}>
        <SpeedTestGauge state={state} value={gaugeValue} />
        {mutation.isPending ? (
          <TestingAnimation label="Checking connection…" />
        ) : result ? (
          <Animated.View
            entering={ZoomIn.duration(animation.duration.normal)}
            style={styles.success}
          >
            <View style={styles.successIcon}>
              <AppIcon name="checkmark" color={colors.textOnAccent} size={20} />
            </View>
      ...
      {mutation.isError ? (
        <Animated.View entering={FadeIn}>
          <SecondaryButton title="TRY AGAIN" onPress={start} />
        </Animated.View>
      ) : null}
      <View style={styles.note}>
        ...
        <AppText style={styles.noteText}>
          Results are mock data in Phase 2B and are saved for this session.
        </AppText>
      </View>
    </AppScreen>
  );
