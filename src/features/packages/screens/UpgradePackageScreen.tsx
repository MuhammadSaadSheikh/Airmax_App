...
export default function UpgradePackageScreen({ route }: Props) {
  ...
  if (customerQuery.isPending || planQuery.isPending || currentQuery.isPending)
    return (
      <AppScreen>
        <AppHeader title="Confirm plan" showBack />
    ...
    return (
      <AppScreen>
        ...
        <ErrorState
          ...
          message="We couldn't prepare this plan change."
          retry={() => {
  ...
  return (
    <AppScreen contentContainerStyle={styles.content}>
      <AppHeader
        title={action === 'renew' ? 'Renew package' : 'Confirm upgrade'}
          ...
          <View style={styles.top}>
            <View style={styles.identity}>
              <AppText style={styles.eyebrow}>
                {action === 'renew' ? 'RENEWING' : 'SELECTED PLAN'}
              </AppText>
      ...
      {action === 'upgrade' && speedDifference > 0 ? (
        <Surface style={styles.improvement}>
          ...
          <View style={styles.improvementCopy}>
            <AppText style={styles.improvementTitle}>
              {speedDifference} Mbps faster
            </AppText>
            <AppText style={styles.improvementText}>
              Compared with your current {current.speed} Mbps package.
            </AppText>
          </View>
        </Surface>
      ) : null}
      <Surface style={styles.summary}>
        ...
        <SummaryRow
          label="Activation"
          value={
            action === 'renew' ? 'After current expiry' : 'Next billing cycle'
          }
        />
      </Surface>
      <View style={styles.notice}>
        ...
        <AppText style={styles.noticeText}>
          This is a preview flow. No package change occurs until backend billing
